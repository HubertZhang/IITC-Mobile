...
// @updateURL      https://secure.jonatkins.com/iitc/release/plugins/default-intel-detail.meta.js
// @downloadURL    https://secure.jonatkins.com/iitc/release/plugins/default-intel-detail.user.js
// @description    [jonatkins-2015-09-17-154202] Use the portal level detail levels from the standard intel site. By default, IITC shows less detail when zoomed out, as this is enough for general use, is more friendly to the niantic servers, and loads much faster. This plugin restores the default zoom level to portal level mapping. Note: using this plugin causes a larger number of requests to the intel server, and at high resolutions can cause excessive requests to be made (yes: the default intel site also has this problem!), so it is not recommended except for low resolution screens.
